...
What it does: computes the next semver from `manifest.json`, writes it into `manifest.json` + `manifest-beta.json` + `package.json` + `versions.json`, writes a changelog entry, runs `npm run build`, commits **all** working-tree changes (`git add -A`) and pushes to the current branch, then `gh release create`s a release whose assets are `dist/{main.js,manifest.json,styles.css}` (the three files BRAT reads).

**Release notes flow one input to two places.** Pass `--notes` and each line becomes a bullet in `src/notices/changelog.json`, keyed by the new version; the script regenerates `src/notices/changelog.ts` (committed, imported by the plugin) from it, and uses the same text as the GitHub release body. The in-app update notice ([version-notices.ts](src/notices/version-notices.ts)) renders that version's bullets with a title derived from the live `manifest.name`/`version` — so it can never show a stale number or another build's notes. With no `--notes`, the entry defaults to "Maintenance release." Don't hand-edit `changelog.ts` (it's auto-generated); edit `changelog.json` or re-release. (A `.ts` module is generated rather than importing the `.json` directly, to avoid enabling `resolveJsonModule`/`allowSyntheticDefaultImports`, which perturb tldraw's prop types.) The release **tag must equal the manifest `version`** or BRAT rejects it — the script guarantees this. It stages everything (not just version files) on purpose: the build compiles the working tree, so committing only version bumps would let the released binary drift from the committed source.
